...
// GET /api/radius/accounting/nas - Get accounting records by NAS IP
export async function GET(request: NextRequest) {
  try {
    ...
    if (!nasIP) {
      return NextResponse.json({ error: "NAS IP address is required" }, { status: 400 })
    }
    ...
    console.error("Error fetching accounting records by NAS IP:", error)
